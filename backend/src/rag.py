...
CONDENSE_INSTRUCTION = (
    ...
    "2. Если реплика уже самостоятельна, верни её ДОСЛОВНО без изменений.\n"
    "3. Не отвечай на вопрос, не добавляй ничего от себя, не меняй язык. "
    "Верни только текст вопроса, без кавычек и пояснений."
)

# Сколько последних пар вопрос-ответ хранить и передавать в конденсацию.
DIALOG_HISTORY_TURNS = 3
...
class WineRAGAssistant:
    ...
    def answer(
        ...
        *,
        ...
        if is_out_of_domain_question(question, self.domain_tokens):
            return RAGAnswer(
                question=question,
