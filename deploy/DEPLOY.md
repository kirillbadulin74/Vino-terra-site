...
Кеш `vector_index/` — производный артефакт и в Git не хранится, поэтому после
клонирования репозитория его нужно собрать один раз:

```bash
cd /opt/vinoterra/backend && sudo -u vinoterra .venv/bin/python build_vector_index.py
```

Шаг требует `OPENAI_API_KEY` в `.env`. Пересобирайте кеш после любых правок базы
знаний. Пока кеша нет, API можно поднять в офлайн-режиме `RAG_MODE=bm25`.
...
## Как развёрнуто на живом сервере

Фактическая установка отличается от инструкции выше: она делалась раньше и
вручную. Её описание — каталоги, порт, пользователь службы, состояние HTTPS — в
этом репозитории намеренно **не хранится**: репозиторий публичный, а конфигурация
сервера к публикации не предназначена.

Описание живой установки и разбор её слабых мест лежат в приватном отчёте на
локальной машине: `Diploma/reports/2026-08-28_server_security_and_coexistence.md`.
Перед правкой конфигов на сервере сверяйтесь с ним, а не только с этой инструкцией.
